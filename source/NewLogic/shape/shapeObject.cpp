...
bool PolygonObject::IsCollisionPolygon(const PolygonObject* pObject) const
{
	...
	std::list<Point>::const_iterator itor1 = pObject->mpointArr.begin();
	std::list<Point>::const_iterator itor2 = itor1; ++itor2;
	std::list<Point>::const_iterator itor_end = pObject->mpointArr.end();
	bool isIns(false);
	while (true)
	{
		...
		if (itor2 == itor_end)
		{
			isIns = IsIntersectionLine(*itor2, *(pObject->mpointArr.begin()));
			break;
		}
		++itor1;
		++itor2;
